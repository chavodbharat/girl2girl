...
class AdMrec extends React.Component {
  ...
  render() {
    ...
    return (
      <View
        style={[GS.flexStretch, GS.flexCol2, styles.cont]}
        ref={cmp => (this.viewCmp = cmp)}>
        <PSMMrecAdView
          ref={cmp => (this.nativeCmp = cmp)}
          {...this.props}
          style={{width: 300, height: 250, backgroundColor: '#ffffff'}}
        />
        {this.state.bg ? (
          <Text style={{color: '#ffffff', fontSize: 1}}>&nbsp;</Text>
        ) : (
          <Text style={{color: '#ffffff', fontSize: 1}}>&nbsp;&nbsp;</Text>
        )}
      </View>
