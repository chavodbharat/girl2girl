...
const Util = {
    ...
    getElapsedTime(date) {
      ...
      //  console.log('now time:', now, ':someDate:',date);
        ...
        if(diffSec < 0) return 'less than 1 min ago';
        let diff = null;
        let unit = null;
        if (diffSec < 60) {
            diff = diffSec;
    ...
    shouldShowInterstitial(cb) {
        Storage.getSceneCount()
            .then(cnt => {
                ...
                // UNSAFE_componentWillReceiveProps call logic (what the
                ...
                //console.log('cnt:',cnt,':Threshold:',SCENE_THRESHOLD);
