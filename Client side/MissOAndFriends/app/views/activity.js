...
import { Image, ListView, RefreshControl, StyleSheet, Text, TouchableHighlight, View } from 'react-native';
...
class ActivityView extends React.Component {
    ...
    _data = (offset = 0) => {
        ...
        Ajax.getUser(this.props.userId, res => {
            this.setState({
                user: res,
            });
            Ajax.getUserConvos(this.props.userId, res => {
    ...
    _renderAvatarRow = () => {
        if (this.state.user && this.state.user.username) {
            ...
            return (
                ...
                >
                    <View style={[GS.flexRow5, GS.flexStretch, GS.bgColorLightPurple, { paddingBottom: 25 }]}>
                        { image }
                        <Text style={[GS.headerText, { paddingTop: 25 }]}>{this.state.user.username}</Text>
                    </View>
                </TouchableHighlight>
            );
        }
    ...
    convoFollow = () => {
        ...
        Ajax.postFollowing(following, res => {
            ...
            this.setState({
                ajaxMsg: `This convo ${this.state.context.text.substring(0, 20)}... will now show up in your feed.`,
                alertType: 'Info',
                async: false,
    ...
    convoUnfollow = () => {
        ...
        Ajax.deleteFollowing(follower_id, following_id, FOLLOWING_TYPES.CONVO, res => {
            ...
            convo.num_followers = convo.num_followers - 1;
            this.setState({
                ajaxMsg: `This convo ${this.state.context.text.substring(0, 20)}... will no longer show up in your feed.`,
                alertType: 'Info',
                async: false,
                rawConvos: this.state.rawConvos,
            }, () => {
    ...
    onUserFollow = () => {
        if (this.sessionUser === null) {
            ...
            this.setState({
                ...
                followFn: this.state.user.is_following ? this.userUnfollow : this.userFollow,
    ...
    userFollow = () => {
        ...
        Ajax.postFollowing(following, res => {
            this.setState({ async: false });
            this.setState({
                ajaxMsg: `Thanks! Your friend request has been sent. You will be notified once it's accepted.`,
                alertType: 'Info',
                context: {
                    ...this.state.context,
                    is_following: true,
    ...
    userUnfollow = () => {
        ...
        const following_id = this.state.context._id;
        this.setState({ async: true });
        Ajax.deleteFollowing(follower_id, following_id, FOLLOWING_TYPES.USER, res => {
            this.setState({ async: false });
            this.setState({
                ajaxMsg: `The latest convos by ${this.state.context.username} will no longer show up in your feed.`,
                alertType: 'Info',
                context: {
                    ...this.state.context,
                    is_following: false,
