...
class Response extends React.Component {
  ...
  constructor(props: any) {
    ...
    if (this.props.response && this.props.response.in_response_to) {
      this.state.inResponseTo = this.props.getRespondedTo(
        this.props.response.in_response_to,
      );
    }
  }

  getStyles = () => {
    return this.getResponse().status == 'publish'
      ? []
      : styles.responseAwaiting;
    //return [];
  };
  ...
  render() {
    return (
      this.getResponse() && (
        <View
          style={[GS.flex1, GS.flexCol1, styles.cont, this.getStyles()]}
          collapsable={false}
          ref={x => {
            if (this.props.xref) {
              this.props.xref(this.props.response._id, x);
            }
          }}>
          <View style={[GS.flexRow1, GS.flexStretch]}>
            <View
              style={[GS.flexRow7, GS.flexGrow1, GS.flexShrink0, GS.flexWrap]}>
              <Text
                onPress={() => {
                  this.activityView(this.getResponderId());
                }}
                style={[styles.smallText, styles.username]}>
                {this.getResponderUsername()}
              </Text>
            </View>
          ...
          <View style={[GS.flexRow1, GS.flexWrap, GS.marginT5]}>
            {this.getResponse().status == 'publish' ||
            (this.state.user &&
              this.state.user._id == this.getResponse().user._id) ? (
              <View
                style={{
                  flexDirection: 'row',
                  flexWrap: 'wrap',
                ...
                {this._renderResponseHandle()}
                {this.mapStringToComponent(this.getText())}
              </View>
            ) : (
  ...
  _renderResponseHandle = () => {
    if (this.getInResponseTo()) {
      ...
      if (this.isResponderTheInResponseToResponder()) {
        return <Text style={[GS.bold]}>... </Text>;
      }
      // otherwise @username link
      return (
        <Text
          onPress={() => {
            this.activityView(this.getInResponseToResponderId());
          }}
          style={[GS.bold]}>
          @{this.getInResponseToResponderUsername()}:{' '}
        </Text>
      );
    }
    return null;
  ...
  getInResponseToResponderUsername = () => {
    ...
    return `${respondedToUser.is_sponsor ? 'our sponsor ' : ''}${
      respondedToUser.username
    }`;
  };
  ...
  gotoUserByName = name => {
    let self = this;
    return () => {
      Ajax.getUserIdByNickname(name.substr(1)).then(id => {
        if (id) {
      ...
      <HTML
        ...
        renderers={{
          user: (att, a) => {
            return (
              <Text
                onPress={self.gotoUserByName(att.name)}
                style={{color: CP.moafDarkPurple}}>
                {a}
              </Text>
            );
          },
          xlink: (att, a) => {
            return (
              <Text
                onPress={self.gotoExtWeb(att.href)}
                style={{color: CP.moafLightPurple}}>
                {a}
              </Text>
            );
          },
          hashtag: (att, a) => {
            return (
              <Text
                onPress={self.searchByTag(att.qs)}
                style={{color: CP.moafSoftPink}}>
                {a}
              </Text>
            );
          },
