...
import { Image, KeyboardAvoidingView, Platform, ScrollView, Text, Alert, TextInput, TouchableHighlight, View } from 'react-native';
...
class RegisterView extends React.Component {
    ...
    constructor(props: any) {
        ...
        this.genderAgeAlertText = 'Only girls 18 and under can join Miss O Girl2Girl Wall';
        this.under13AlertText = "Because you are under 13, we need to get permission from your parent. Please Enter your parent or guardian's email address.";
        this.platform = Platform.OS;
        this.registrationAlertText        = 'Check your email to complete registration.';
        this.registrationUnder13AlertText = 'Ask you parent or guardian to check their email to complete registration.';
        this.emailExistsMessage           = 'This email has already been used. Please try another one.';
        this.usernameExistsMessage        = 'This username already exists. Please use another one.';
    }
                ...
                >
                    ...
                    >
                        <Text style={[GS.select]}>
                            {
                                !this.state.selectedDob ? <Text
                                    style={[GS.selectPh]}>date of birth</Text> : Util.formatDate(this.state.selectedDob)
                            }
                        </Text>
                    </TouchableHighlight>
                    <ValidationError
                    ...
                    <View style={[GS.flexCol5, GS.flexStretch, GS.marginT10, GS.paddingV5]}>
                        <Text style={[GS.text, GS.colorWhite]}>
                            Because you are under 13, we need to get permission from your parent or guardian so that you can participate in certain activities on the site as a regular Club Member. Please Enter your parent or guardian's email address.
                        </Text>
                    </View>
                    <TextInput
                        autoCapitalize={'none'}
                    ...
                    />
                    <View style={[GS.flexCol5, GS.flexStretch, GS.marginT10, GS.paddingV5]}>
                        <Text style={[GS.text, GS.colorWhite]} onPress={this.login}>Already have an account? <Text
                            style={[GS.bold]}>Log in</Text></Text>
                    </View>
                </ScrollView>
                ...
                >
                    <Text style={[GS.select]}>
                        {!this.state.selectedCountry ?
                            <Text style={[GS.selectPh]}>country</Text> : this.state.selectedCountry.name}
                    </Text>
                </TouchableHighlight>
            );
        } else {
            ...
            if (this.platform === 'ios') {
                return (
                    <MoafPicker
                        ...
                        selectedKey={this.state.selectedGender ? this.state.selectedGender.id : null}
                    />
                );
            }
        }
                    ...
                    <Text style={[GS.select]}>
                        {
                            !this.state.selectedGender ?
                                <Text style={[GS.selectPh]}>gender</Text> : this.state.selectedGender.name
                        }
                    </Text>
                </TouchableHighlight>
            );
                            ...
                            <Text style={[GS.select]}>
                                {
                                    !this.state.selectedUsaState ?
                                        <Text style={[GS.selectPh]}>state</Text> : this.state.selectedUsaState.name
                                }
                            </Text>
                        </TouchableHighlight>
                        <ValidationError
    ...
    isRegistrationDisabled = () => {
        const {
            confirmInError, countryInError, dobInError, emailInError, genderInError, nameFirstInError, passwordInError, usernameInError
        } = this.state;
        if (confirmInError || countryInError || dobInError || emailInError || genderInError || nameFirstInError || passwordInError || usernameInError) {
            this.setState({ registrationInError: true });
            return;
        }
        const {
            confirmPassword, selectedCountry, selectedDob, selectedGender, selectedUsaState, user
        } = this.state;
        if (!confirmPassword || !selectedCountry || !selectedDob || !selectedGender) {
            this.setState({ registrationInError: true });
            return;
        }
    ...
    onChangeConfirmPassword = (text: string) => {
        ...
        if (inError) {
            this.setState({
                confirmMsg: text.trim() === '' ? 'password confirmation required' : 'must match password',
            });
        }
    };
    ...
    onClosedAlertDialog = () => {
        if (this.state.ajaxMsg === this.registrationAlertText || this.state.ajaxMsg === this.registrationUnder13AlertText) {
    ...
    onSelectedDob = (obj: Date) => {
        ...
        Schema.validate(obj, Schema.dob, 'date of birth', (err) => {
            ...
            if (err) {
                this.setState({
                    dobMsg: tooOldRegex.test(err) ? 'you must be 18 years or younger' : err,
                });
    ...
    register = () => {
        ...
        if (this.state.selectedCountry.alpha_3 === 'USA' && this.state.selectedUsaState) {
        ...
        Ajax.usernameExists(payload.username)
        .then((exists) => {
            if (exists) {
                self.setState({
    ...
    postUser = (payload) => {
        Ajax.postUser(payload, () => {
            let msgText = this.state.showParentEmail ? this.registrationUnder13AlertText : this.registrationAlertText;
            this.setState({
                ajaxMsg: msgText,
                alertTitle: '',
                alertType: 'Info',
