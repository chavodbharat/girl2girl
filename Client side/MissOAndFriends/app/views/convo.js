...
import { Alert, Image, InteractionManager, ListView, Modal, RefreshControl, ScrollView, Share, StyleSheet,
    Text, TextInput, TouchableHighlight, View, WebView } from 'react-native';
...
class ConvoView extends React.Component {
    ...
    constructor(props: any) {
        ...
        this.state = {
            ...
            responses: Util.newListViewDataSource(),
            isSubmitted: false,
            willFollow: false,
        };
            ...
            if (self.props.scrollTo) {
                ...
                if (self.vrefs.hasOwnProperty(tmp)) {
                    ...
                    self.vrefs[tmp].measure((fx, fy, width, height, px, py) => {
                        if (py != undefined) {
                            self.slv.scrollTo({x: 0, y: py, animated :true});
                        } else {
                            let irv = setInterval(() => {
                                self.slv.scrollTo({x: 0, y: i += 20, animated :true});
                                self.vrefs[tmp].measure((fx, fy, width, height, px, py) => {
                                    if (py) {
                                        self.slv.scrollTo({x: 0, y: py + height + 100, animated :true});
                                        clearInterval(irv);
                                    }
                                });
                            }, 1);
        ...
        if (this.state.convo) {
            return (
                <View style={[GS.flex1, GS.flexStretch, GS.marginT6]} >
                    ...
                    <View style={[GS.flexCol1]}>
                        ...
                        <View style={[GS.flexRow1, GS.paddingH10]}>
                            <View style={[GS.flexRow7, GS.flexGrow1, GS.flexShrink0, GS.flexWrap]}>
                                <Text style={[GS.smallText, styles.darkGrayText]}>Added by</Text>
                                <Text style={[GS.smallText, GS.username]}
                                      onPress={this.activityView}> {this.getUsername()}</Text>
                            </View>
                    ...
                    <View style={[GS.flexRow1, GS.flex1, GS.marginV10, GS.paddingH10, { maxHeight: 100 }]}>
                        <ScrollView
                        ...
                        >
                            <WebView source={{html: this.getStyledText(), baseUrl: ''}}  originWhitelist={['*']} style={[{height: 90}]} scalesPageToFit={(Platform.OS === 'ios') ? false : true}/>
                        </ScrollView>
                    </View>
                    ...
                    <View style={[styles.convoMetricsCont, GS.flexStretch]}>
                        <Text style={[styles.convoMetricsText]}>
                            {this.getNumberOfResponsesText()} <Text
                            style={[styles.lightGrayText]}>• {this.getFollowersText()}</Text>
                        </Text>
                    </View>
        ...
        return (
            ...
            >
                <View style={[styles.modal]}>
                    <View style={[{ backgroundColor: 'rgba(255,255,255,1.0)', flex: 1, maxHeight: 225 }]}>
                        <View style={[GS.flexStretch, GS.flexRow5, GS.bgColorSkyBlue, GS.paddingV5]}>
                            <Text style={[styles.btnText]}>{this.getUsername()}</Text>
                        </View>
                        <View style={[GS.flexStretch, GS.flexRow1, GS.flex1, { maxHeight: 100 }]}>
                            <ScrollView
                                keyboardShouldPersistTaps={'handled'}
                                showsHorizontalScrollIndicator={false}
                                style={[GS.flexStretch, GS.paddingH10, { height: 90 }]}
                            >
                                <Text style={[styles.btnText, GS.colorBlack]}>{this.getText()}</Text>
                            </ScrollView>
                        </View>
                        <View>
                            <ResponseInput
                                ref={ responseInput => { this.responseInput = responseInput } }
        ...
        return (
            ...
            >
                <View style={[styles.modal]}>
                    <View style={[{ backgroundColor: 'rgba(255,255,255,1.0)', flex: 1, maxHeight: 225 }]}>
                        <View style={[GS.flexStretch, GS.flexRow5, GS.bgColorSkyBlue, GS.paddingV5]}>
                            <Text style={[styles.btnText]}>{this.state.respondedToUsername}</Text>
                        </View>
                        <View style={[GS.flexStretch, GS.flexRow1, GS.flex1, { maxHeight: 100 }]}>
                            <ScrollView
                               keyboardShouldPersistTaps={'handled'}
                               ...
                               style={[GS.flexStretch, GS.paddingH10, { height: 90 }]}
                            >
                                <Text style={[styles.btnText, GS.colorBlack]}>{this.state.respondedToText}</Text>
                            </ScrollView>
                        </View>
                        <View>
                            <ResponseInput
								...
								ref={ respondeInput => { this.respondeInput = respondeInput } }
								onTextChange = { this.onResponseTextChange }
    ...
    _renderResponses = () => {
                    ...
                    <ListView
                        ...
                        renderRow={response => (
                                ...
                                <Response
                                    ...
                                    onResponse={this.openResponseTo.bind(this)}
        ...
        return (
            <View style={[GS.bgColorWhite, GS.flex1, GS.flexCol1, { overflow: 'visible' }]}>
                <View style={[GS.flexCol2, GS.flexStretch, GS.padding15, GS.flexWrap]}>
                    <Text style={[styles.noneText, GS.bold]}>No Chats Yet</Text>
                </View>
                {this._renderFollowText()}
                <View style={[GS.flexCol2, GS.flexStretch, GS.paddingH15, GS.flexWrap]}>
                    <Text
                        onPress={this.onRespond}
    ...
    _data = () => {
        ...
        Ajax.getConvo(this.props.convoId, res => {
            const responses = [].concat(res.responses.sort((a, b) => {
                if (a.status == 'pending') {
                    if (b.status != 'pending') {
                        return -1;
                    }
            ...
            this.setState({
                convo: res,
                responses: Util.newListViewDataSource().cloneWithRows(responses),
            });
    ...
    activityView = () => {
        Actions.activity({ adCheckToken: Util.randomNumber(), userId: this.getConvoUserId() });
    };
    ...
    getStyledText = () => {
        return '<style>html {font-family: "Raleway-Regular"; font-weight: 400; font-size: 15}</style><html>' + this.state.convo.text + '</html>';
    };

    getUsername = () => {
        return `${this.state.convo.user.is_sponsor ? 'our sponsor ' : ''}${this.state.convo.user.username}`;
    };
    ...
    inResponseTo = text => {
        ...
        this.setState({
            ...
            Ajax.postResponse(this.state.convo._id, response, res => {
                this.setState({
                    ajaxMsg: 'Thanks! Your chat was received and will be posted v v soon!!',
                    alertType: 'Info',
                    isSubmitted: false,
                    async: false
                }, () => {
    ...
    openResponseTo = _id => {
        ...
        const inResponseToUsername = `${inResponseTo.user.is_sponsor ? 'our sponsor ' : ''}${inResponseTo.user.username}`;
        this.setState({
    ...
    share = responseText => {
        this.setState({ async: true });
        Ajax.getConvoUrl(this.state.convo._id, json => {
            this.setState({ async: false });
            ...
            if (responseText) {
                ...
                text = `Check out this convo ${Util.sanitize(convoText)} on MissOAndFriends!`;
            }
            Share.share({
